...
def demo_fetch_mode():
    ...
    print("\nFetching: fixed.income/govies/treasury/US/10Y/ALL (limit=5)")
    print("The service executes the query and returns actual data.\n")

    resp = httpx.get(f"{BASE_URL}/fetch/fixed.income/govies/treasury/US/10Y/ALL?limit=5")
    data = resp.json()
...
def demo_compare_modes():
    ...
    path = "fixed.income/govies/treasury/US/10Y/ALL"
    print(f"\nMoniker: {path}\n")
