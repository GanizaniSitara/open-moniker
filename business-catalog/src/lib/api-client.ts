 ...
 * All catalog data is fetched from the service API rather than YAML files.
 */
...
/**
 * Fetch all node summaries in paginated batches.
 * Returns lightweight data suitable for listing pages.
 */
export async function fetchNodeSummaries(): Promise<ApiNodeSummary[]> {
  const first = await apiFetch<NodeSummaryListResponse>(
    `/config/nodes?summary=true&offset=0&limit=${SUMMARY_PAGE_SIZE}`
  );
